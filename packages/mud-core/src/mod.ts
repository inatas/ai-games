...
export interface ModDefinition {
  ...
  npcs: { id: string; roomId: string }[]; actions: string[];
}
...
export class ModRegistry {
  ...
  register(definition: ModDefinition) {
    ...
    if (!unique(definition.npcs.map(n => n.id)) || definition.npcs.some(n => !rooms.has(n.roomId))) throw new Error('Invalid NPCs');
