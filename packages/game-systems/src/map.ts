...
export function projectMap(value: MapDefinition, current: string, discovered: Set<string>, access: (exit: Exit) => string | null) {
  let index = indices.get(value);
  if (!index) { validateMap(value, current); index = indices.get(value)!; }
