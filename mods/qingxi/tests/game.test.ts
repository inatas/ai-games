...
async function read(g:{id:string;token:string}){return (await built.app.inject({url:`/api/mud/current`,headers:{cookie:g.token,origin:'http://localhost:80'}})).json();}
async function act(g:{id:string;token:string},action:string,version:number,id=randomUUID()){
 // These rule fixtures supply the required location. Actual travel is covered by room-game.test.ts.
 ...
 const r=await built.app.inject({method:'POST',url:`/api/mud/actions`,headers:{cookie:g.token,origin:'http://localhost:80'},payload:{requestId:id,expectedMemoryVersion:version,action,note:''}});
...
test('D-18 and F-30: idempotency precedes game eligibility, cross-session access denied',async()=>{
 ...
 const forbidden=await built.app.inject({url:`/api/mud/current`,headers:{cookie:'harness_session=wrong'}});assert.equal(forbidden.statusCode,401);
 const malformed=await built.app.inject({method:'POST',url:`/api/mud/actions`,headers:{cookie:g.token,origin:'http://localhost:80'},payload:{action:'execute-sql'}});assert.equal(malformed.statusCode,400);
});
