...
type Pending={requestId:string;expectedMemoryVersion:number;action:string;note:string};
type Game={id:string;state:{silver:number;virtue:number;skill:number;master:string|null;hostVersion:number;encounterDone:boolean};memoryVersion:number;events:any[];openItems:any[]};
...
function App(){
  ...
  <section className="intro"><div className="eyebrow">GAME AI HARNESS · MVP</div><h1>行一步江湖，<br/><em>留一段来日。</em></h1><p>往事有迹，抉择有因。一个用持续记忆串起的微型江湖。</p></section>
...
function GameView({session,restore,leaving}:{session:UserSession;restore:()=>Promise<void>;leaving:boolean}){
  ...
  async function refresh(){const current=await api(`/api/wuxia/games/${session.currentScopeId}`);setGame(current);}
  ...
  function act(action:string){if(!game||pending||busy.current||resetRequest||starting||leaving)return;busy.current=true;setError('');const payload={requestId:crypto.randomUUID(),expectedMemoryVersion:game.memoryVersion,action,note};localStorage.setItem(pendingKey,JSON.stringify(payload));setPending(payload);}
  return <><p className="mode"><i/>{mode==='mock'?'演示模型 · 固定判定':'真实模型'}</p>
    ...
    {!game?<section className="welcome"><p>{error||'正在读取长期存档…'}</p>{error&&<button onClick={()=>void restore()}>重试读取</button>}</section>:<div className="layout"><main>
      <div className="section-head"><h2>此刻的选择</h2><span>已记录 {game.memoryVersion} 次行动</span></div>
      <div className="actions">{choices.map(c=><button key={c.id} className="action" onClick={()=>act(c.id)} disabled={!!pending||!!resetRequest||starting||leaving}><span className="seal">{c.symbol}</span><div><strong>{c.name}</strong><p>{c.description}</p><small>{c.kind}</small></div><span className="arrow">↗</span></button>)}</div>
      ...
      {pending&&<div className="notice" role="status">正在处理「{choices.find(c=>c.id===pending.action)?.name}」… 结果确认后才会写入江湖。</div>}
      {error&&<div className="error" role="alert">{error}</div>}
      <div className="section-head history-head"><h2>江湖旧事</h2><span>已发生的事实</span></div>
