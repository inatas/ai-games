...
基线main/1147166；按用户授权移除mud-core包、旧武侠API、游客凭据及历史数据搬运。Docker类型检查、仓库检查、Vite构建和全量61/61测试通过，无失败或跳过；比上一轮少的两项为已取消的旧NPC冲突迁移与旧社交摘要兼容测试。保留并补强空库结构、NPC/库存重复初始化、跨账号及退休角色请求隔离测试。

标准Dockerfile构建app成功；明确核实game-ai-harness项目harness数据库后清空public schema并重新初始化。清理前账号1、角色1、请求13；部署后账号0、角色0、初始NPC5，旧库存/NPC/社交/迁移表不存在。app/postgres healthy，宿主127.0.0.1:3100首页200、健康接口ok（mock）。独立postgres-test已停止。没有新建清库前备份，此前.local/backups历史备份未删除；恢复历史备份需匹配历史代码，不能作为新结构升级输入。本轮未验证外部设备访问或真实模型。

## 当前三层架构验收（2026-09-17）

platform/game-systems/MOD拆分后，Docker全量63/63通过，类型、仓库边界与前端构建通过。新增无地图基础平台、钱包、库存与策略事务测试。备份隔离恢复后重复迁移保持原数据摘要，详情见[需求013](../.agents/note/013-three-layer-platform.md)。以下历史记录不代表当前功能缺口；真实支付与跨realm账号IM尚未实现。
